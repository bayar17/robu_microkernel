...
void _start(void) {
    ...
    for (int i = 0; i < SOCK_TEST_POLL_MAX; i++) {
        int64_t rc = sock_connect_call(sockid, SOCK_TEST_PATH);
        if (rc == IPC_ERR_NONE) {
            connected = 1;
            break;
        }
    ...
    for (int i = 0; i < SOCK_TEST_PATTERN_LEN; i++) {
        txbuf[i] = (uint8_t)(i ^ 0x5A);
    }
    int total_tx = 0;
    while (total_tx < SOCK_TEST_PATTERN_LEN) {
        int n = 0;
        if (sock_write_call(sockid, txbuf + total_tx, SOCK_TEST_PATTERN_LEN - total_tx, &n) != IPC_ERR_NONE) {
            break;
        }
        total_tx += n;
    }
    ...
    for (int i = 0; i < SOCK_TEST_POLL_MAX && total_rx < SOCK_TEST_PATTERN_LEN; i++) {
        int n = 0;
        int64_t rc = sock_read_call(sockid, rxbuf + total_rx, SOCK_TEST_PATTERN_LEN - total_rx, &n);
        if (rc == IPC_ERR_NONE) {
            total_rx += n;
        }
