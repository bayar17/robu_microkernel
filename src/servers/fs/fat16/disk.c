...
static uint16_t fat_read_entry(uint32_t cluster) {
    ...
    return (uint16_t)fatbuf[entry_offset] | ((uint16_t)fatbuf[entry_offset + 1] << 8);
}

static int fat_write_entry(uint32_t cluster, uint16_t value) {
    static uint8_t fatbuf[512];
    uint32_t fat_offset = cluster * 2;
    uint32_t sector_in_fat = fat_offset / g_fat.bytes_per_sector;
    uint32_t entry_offset = fat_offset % g_fat.bytes_per_sector;
    for (uint32_t copy = 0; copy < g_fat.num_fats; copy++) {
        uint32_t fat_sector = g_fat.fat_start + copy * g_fat.sectors_per_fat + sector_in_fat;
        if (blkdev_read(fat_sector, 1, fatbuf) != 0) {
            return -1;
        }
        fatbuf[entry_offset] = (uint8_t)value;
        fatbuf[entry_offset + 1] = (uint8_t)(value >> 8);
...
static int walk_chain(uint16_t first_cluster, fat16_handle_t *h) {
    h->num_clusters = 0;
    uint32_t cluster = first_cluster;
    int iters = 0;
    while (cluster >= 2 && cluster < FAT16_EOC && iters < FAT16FS_MAX_CLUSTERS * 4) {
        if (h->num_clusters >= FAT16FS_MAX_CLUSTERS) {
            return -1;
        }
        h->clusters[h->num_clusters++] = cluster;
...
static void free_chain(uint16_t first_cluster) {
    uint32_t cluster = first_cluster;
    int iters = 0;
    while (cluster >= 2 && cluster < FAT16_EOC && iters < FAT16FS_MAX_CLUSTERS * 4) {
        uint16_t next = fat_read_entry(cluster);
...
static int alloc_cluster(uint32_t *out_cluster) {
    static uint8_t fatbuf[512];
    uint32_t entries_per_sector = g_fat.bytes_per_sector / 2;
    for (uint32_t s = 0; s < g_fat.sectors_per_fat; s++) {
        ...
        for (uint32_t e = 0; e < entries_per_sector; e++) {
            ...
            uint16_t val = (uint16_t)fatbuf[e * 2] | ((uint16_t)fatbuf[e * 2 + 1] << 8);
            if (val == 0x0000) {
                if (fat_write_entry(cluster, FAT16_EOC_MARK) != 0) {
                    return -1;
                }
                *out_cluster = cluster;
                return 0;
...
static int find_in_root(const uint8_t name83[11], uint16_t *out_cluster, uint32_t *out_size,
    ...
    for (uint32_t s = 0; s < g_fat.root_dir_sectors; s++) {
        if (blkdev_read(g_fat.root_dir_start + s, 1, dirbuf) != 0) {
            return -1;
        }
        ...
        for (int e = 0; e < per_sector; e++) {
            ...
            if (match) {
                *out_cluster = d->first_cluster_low;
                *out_size = d->file_size;
                *out_dirent_sector = g_fat.root_dir_start + s;
    ...
    for (uint32_t s = 0; s < g_fat.root_dir_sectors; s++) {
        ...
        for (int e = 0; e < per_sector; e++) {
            ...
            if (d->name[0] == 0x00 || d->name[0] == 0xE5) {
                *out_sector = g_fat.root_dir_start + s;
...
static int update_dirent_times(fat16_handle_t *hd, int64_t atime, int64_t mtime, uint64_t flags) {
    static uint8_t dirbuf[512];
    if (blkdev_read(hd->dirent_sector, 1, dirbuf) != 0) {
        return -1;
    }
    fat_dirent_t *d = (fat_dirent_t *)(dirbuf + hd->dirent_offset);
    d->first_cluster_low = hd->num_clusters > 0 ? (uint16_t)hd->clusters[0] : 0;
    d->first_cluster_high = 0;
...
static int update_dirent(fat16_handle_t *hd) {
    int64_t now = fat_now();
    return update_dirent_times(hd, now, now, 0);
}

static int fat16_mount(void) {
    if (blkdev_probe() != 0) {
        return -1;
    }
    ...
    uint16_t root_entry_count = (uint16_t)bpb[17] | ((uint16_t)bpb[18] << 8);
    uint16_t tot_sec16 = (uint16_t)bpb[19] | ((uint16_t)bpb[20] << 8);
    uint16_t sectors_per_fat = (uint16_t)bpb[22] | ((uint16_t)bpb[23] << 8);
    ...
    g_fat.root_dir_sectors = ((uint32_t)root_entry_count * 32 + bytes_per_sector - 1) / bytes_per_sector;
    g_fat.root_dir_start = g_fat.fat_start + (uint32_t)num_fats * sectors_per_fat;
    g_fat.data_start = g_fat.root_dir_start + g_fat.root_dir_sectors;
    ...
    if (g_fat.total_clusters < 4085 || g_fat.total_clusters > 65524) {
        return -1;
    }
    return 0;
}
