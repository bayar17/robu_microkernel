...
static void handle_open(msg_regs_t *m) {
    ...
    const char *rel = path + FAT32FS_PREFIX_LEN;
    uint8_t name83[11];
    to_83(rel, name83);
    ...
    int hidx = -1;
    for (int i = 0; i < FAT32FS_MAX_HANDLES; i++) {
        if (!handles[i].in_use) {
            hidx = i;
            break;
        }
    ...
    fat32_handle_t *hd = &handles[hidx];
    hd->in_use = 1;
    hd->file_size = size;
...
static void handle_read(msg_regs_t *m) {
    ...
    if (h >= FAT32FS_MAX_HANDLES || !handles[h].in_use) {
        reply->status = VFS_ERR_BAD_HANDLE;
        return;
    }
    fat32_handle_t *hd = &handles[h];
...
static void handle_write(msg_regs_t *m) {
    ...
    while (got < len) {
        ...
        if (cluster_idx >= (uint32_t)hd->num_clusters) {
            if (hd->num_clusters >= FAT32FS_MAX_CLUSTERS) {
                break;
            }
            ...
            if (hd->num_clusters > 0) {
                if (fat_write_entry(hd->clusters[hd->num_clusters - 1], new_cluster) != 0) {
                    break;
                }
            }
...
static void handle_utimens(msg_regs_t *m) {
    ...
    if (req->handle >= FAT32FS_MAX_HANDLES || !handles[req->handle].in_use ||
